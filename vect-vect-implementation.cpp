...
int chooseVar(std::vector<int> truth_vals, vector<vector<int>> &formula)
{
    ...
    // priority_queue<int, vector<int>, greater<int>> poss_choices;
    // for (auto clause : formula){
    //     int sz = clause.size();
    //     for (int i = 1; i < sz; i++){
    //         if (truth_vals[clause[i]] == -1) poss_choices.push(abs(clause[i]));
    //     }
    // }
    // return poss_choices.top();

    // cout << num_decisions << endl;
    // std::vector<int> unassigned;
    // for (int i = 1; i < truth_vals.size(); i++)
    // {
    //     if (truth_vals[i] == -1 && i == poss_choices.top())
    //         return i;
    // }
...
vector<int> DPLL(std::vector<vector<int>> formula, std::vector<int> truth_vals)
{
    ...
    vector<vector<int>> temp = formula;
    vector<int> sat = UnitProp(temp, truth_vals);
